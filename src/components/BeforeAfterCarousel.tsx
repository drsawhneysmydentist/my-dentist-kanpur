...
// Same proven autoplay/drag engine as the "Glimpse" carousel (ImageCarousel),
// adapted so each card is sized off the image's OWN aspect ratio instead of a
// single forced ratio — every Before & After photo stays fully visible,
// never cropped. Card height is fixed per breakpoint (matching the Glimpse
// card heights) so the row still reads as one consistent, premium strip;
// width simply follows each image's true proportions.
const CARD_HEIGHT_CLASS = "h-[62.4vw] sm:h-[44vw] md:h-[256px] lg:h-[240px] xl:h-[224px]";
const GAP = 20; // px, must match the gap used on the track
const BASE_SPEED = 95; // px/sec — matches the Glimpse carousel speed
...
function toDisplayImage(c: BeforeAfterCase): DisplayImage {
  return {
    id: c.id,
    src: c.thumbnail ?? c.afterImage,
    alt: `${c.treatment} — before & after${c.patientName ? `, ${c.patientName}` : ""}`,
    ratio: c.ratio ?? 1,
  };
}

export function BeforeAfterCarousel() {
  const images = useMemo(() => beforeAfterCases.map(toDisplayImage), []);
  ...
  const loopedImages = useMemo(() => {
    const out: Array<DisplayImage & { _key: string; _origIndex: number }> = [];
  ...
  return (
    <section className="overflow-hidden px-3 py-20 md:px-5 md:py-28" aria-label="Before and after transformations">
      <Reveal>
        <div className="mx-auto mb-10 flex max-w-7xl flex-wrap items-end justify-between gap-6 px-3 md:mb-12">
          <div>
            ...
            <h2 className="mt-6 max-w-xl font-display text-4xl md:text-5xl">
              Premium <span className="italic text-aqua-deep">Before &amp; After</span> Transformations
            </h2>
          </div>
          ...
          {loopedImages.map((img, i) => {
            ...
            return (
              <div
                ...
                className={`group relative shrink-0 overflow-hidden rounded-[1.75rem] bg-bone shadow-soft md:rounded-[2rem] ${CARD_HEIGHT_CLASS}`}
